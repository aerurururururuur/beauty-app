...
> 以「风景」为灵感的 AI 上妆应用：上传**本人正面照**，再给一个**场景**（风景图片 和/或 一句描述文字），AI 依据场景氛围去参考妆面，为这张照片配一套合适妆容——最终把妆容直接渲染到照片上。

当前为**可运行骨架**：引擎 / 场景理解 / 参考图检索均为 mock 实现，接口与分层已按可替换接缝留好，换真实实现不改业务层。

## 业务流程

```
本人照片 ─┐
           ├─► ① 场景理解 ─► ② 按场景检索参考妆 ─► ③ 选妆并渲染到照片 ─► ④ 返回结果图
场景(图/文)┘        scene            references            engine             result
```
...
curl -s http://localhost:3000/api/jobs/<id>  # 轮询到 done
...
| 子目录 | 职责 |
| --- | --- |
| `entities/` | 领域实体与纯函数（Job 状态机、Scene/Reference/Look 等） |
...
真实妆容渲染引擎、视觉大模型场景理解、真实参考图抓取、用户系统与鉴权、削峰队列与多机/云存储。
