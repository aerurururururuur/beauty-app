...
> ✏️ **2026-09-16：这条规矩的第二处应用出现了**——`agent` 要用同一句话
> （`POST /agent/sessions` 不许给不存在的用户开会话）。照抄的不只是做法，还有一个细节：
> **两份端口类型是各自声明的**（`cabinet/domain/ports/user-directory.ts` 与
> `agent/domain/ports/user-directory.ts`，**同名同形但不是同一个类型**）——
> 复用那个类型会让两个业务模块的 domain 在编译期互认，正是本页禁止的。
> 而**组装根那个闭包只有一份**，两个模块共用：因为"什么算用户不存在"
> （只把 `USER_NOT_FOUND` 翻译成 `false`、存储故障照抛）是一个会分叉的判断。
> ⇒ **形状相同 ⇒ 可以共用实现；类型相同 ⇒ 就成了一条不该有的依赖。** 两者别混。
...
| 推荐 / 品牌参考 | **2026-09-16 起已有实现**（不是规则引擎，是模型读内容库）：`products` 模块 + agent 的 `list_products` / `read_product` | `agent/domain/ports/product-library.ts`（第三处 §7.1 粘合） | 没配 `PRODUCTS_DIR` 就不注册这两个工具（**是可关闭的**，见 §9）。★ 原设想的**规则引擎**（衣橱缺口 → 查表）**仍未做**，那是另一条 |
...
- **待办 [ ]（wow 的唯一来源 = 上妆像本人、且自然）**：
  - [ ] **拍板（阻塞下面两项）**：① 自研参数化渲染（关键点 + 局部调色合成，可控、原创强） vs ② 第三方上妆图像 API（快而稳，需核授权、原创叙事弱）。**选型方向 + 真实 API 实测规范见 `docs/plan/ai-engine-api-spike.md`（可整单派人）**——先按那里以 Perfect Corp Copy Makeup 为主（个人免企业、¥0 免费单元够 hackathon；美图需企业认证已放弃为主选），再回来勾这一项。
    - **⚠️ 2026-09-12 · 主选作废，本项重新打开**：owner 拍板**放弃 Perfect Corp，理由 = 成本**。
      至此**没有主选**，上面那句「先按 Perfect Corp 为主」**不再成立**，`ai-engine-api-spike.md`
      也从「可整单派人」变成「不可派单」。该文 **§0** 记了作废范围与一条**待回填的矛盾**
      （该文 §1/§3 声称「¥0 免费单元够 hackathon」，与「太贵用不起」冲突，回填前不得引用）。
      - **✅ 2026-09-12 同日回填完成**：矛盾已解决，**是 §1 原文的数字错了，不是 owner 判断错了**。
        注册免费档是 **40 units**（三个独立来源一致），不是 500~1000——那俩是促销码/黑客松专属。
        40 units = 20 张 Copy Makeup，**跑不完 §4.3 的调用矩阵**。owner 已确认原因**正是费用**
        （原话「不要了，用不起」），该文 §0.1 的 `【待 owner 确认】` 就此关闭。
        **§1 那句「¥0 够用」现在可以引用，但只能引成「额度小到只够预录 demo」**，
        不能引成「不用花钱所以可以当主选」——原文已加指引，见该文 §0.1。
      **不受影响的**：§3.1 对四个生成式候选的排除、`ai-engine-selfhost-review.md` 对自建链的排除
      —— 那些结论与价钱无关，仍然有效。
      **代价要说清**：`api-spike` §4 那份可派单的实测规范是**为 Perfect Corp 专门写的**，
      作废后没有替代品——重开选型等于把这一单的工期退回起点。但 §4.2 夹具 / §4.4 评分表 /
      §4.5 record-replay / §4.6 隐私红线 / §4.7 可达性第 0 步**这五件当初就是按厂商无关写的，可原样复用**。
      **下面几条 09-11 的补充仍然有效**（它们讲的是路线形状，不是具体厂商），但其中凡引用
      「主选厂商」的地方，现在都只能读成「假设存在这样一家厂商」。
    - **一处必须盯住的连带损失**：§13-2「`references` 没有合规素材来源」的**唯一出路**
      （走参数化接口 → 参考图退役 → 悬空项消掉）是**挂在 Perfect Corp 上的**。厂商一走，这条出路一并作废，
      **红线 §13-2 退回完全未解决状态**，见 §13-2 该条。
    - **2026-09-11 补充**：主选厂商另有**参数化上妆接口** AI Makeup VTO，**1 unit/张、不吃参考图**，
      LLM 可直接输出 `effects[]` 参数 JSON，形状与 `Look{style,palette,zones}` 近乎同构，见该文 **§1.2**。
      它对本项目最大的价值不是审美而是**合规**：参考图一旦不再是必需输入，`references` 可降为可选，
      §13-2 那个「没有合规素材来源」的悬空项随之消掉。**「prompt 驱动要换供应商」这个前提不成立。**
    - 同期复核的四个生成式候选 MAD / SD+FaceID / FLUX+LoRA / Picsart **均不建议作主线**，理由见该文 **§3.1**；
      SDXL / ControlNet / IP-Adapter FaceID 这条**自建链**的完整论据见 **`docs/plan/ai-engine-selfhost-review.md`**：
      本机实测 RTX 5060 Laptop **8 GB** 而整条链驻留 >13 GB、两条腿踩 InsightFace **NC**、人脸分割无腮红/眼影类。
    - **选型框架已变（2026-09-11 收敛）**：上条「① 参数化渲染 **vs** ② 第三方 API」的对立关系不成立了。
      已收敛的落地路线是 **ComfyUI + SD Inpainting + Color Guide + 分区 denoise**，见该文 **§5**，
      其中「参数化渲染」不再是终点，而是喂给扩散模型的 color guide 生成步骤，**① 与 ③ 合并了**。
      下面两条待办的角色随之改变：关键点检测仍是前置；参数化渲染仍要做，但产出的不再是成片而是 color guide。
  ...
  - [ ] **参数化渲染**：把妆容画到照片像素（唇 / 眼影 / 底妆调色合成），保「素颜真实度」，不做夸张滤镜；实现后产物仍交 `engine-output.validator` 把关。
    - **角色已改（2026-09-11）**：走自建路线时它产出的是 **color guide**（不是成片），
      成片由扩散模型在它上面做 inpaint 得到——见 `ai-engine-selfhost-review.md` §5.1 / §5.3。
  - [ ] **（2026-09-11 二次检索）本地模型出现一条「许可干净 + 8 GB 跑得动」的强选项**：
    **Qwen-Image-Edit-2509 是 Apache 2.0，配 Nunchaku 的 INT4 推理能在本机跑**（Nunchaku 同为 Apache 2.0），
    唯一待验的是 4-bit 下唇部细节保不保得住。**FLUX-Makeup 虽最对症，但骨干是 Flux-Kontext.dev，非商用，排除**——
    「看骨干不看包装」这条判据已写进 `ai-engine-selfhost-review.md` §5.6。验证步骤见该文 §5.8 阶段 5。
  - [ ] **`Look` 加逐部位 `finish` 质地字段**（2026-09-11 新开，**阻塞自建路线**）：
    现有 `Look{ style, palette, zones }` **没有任何地方放「satin / matte / glossy」**，
    而这恰恰是「像涂的 vs 像染的」的分水岭，见自建复核文 §5.4。加一个枚举。
    **改动面按 §2「枚举只定义一处」的规矩走**：`shared` 枚举 → zod schema → `MockEngine` →
    `application/narration.ts` → 前端 chips → 测试，**一处不能漏**。
  - [ ] **`docs/环境搭建-Windows版.md` 补 Python + ComfyUI 一节**（2026-09-11 新开）：
    该文档现在写着「没有 Python，统统不用装」，采纳自建路线后这句变成过期描述，
    不补的话下一个照它装环境的人会卡住。
  - [ ] **保留 mock 分支作离线兜底**：演示永不因引擎崩掉（`config.makeupEngine` 分发，`mock` 常驻可选）。
  - [ ] 真实渲染变慢后，核对 jobs 前端等待体验（见 §7 最后一项）与 narration 文案是否贴合真实妆效。
  - [ ] **★ 接引擎 + 做对话式 agent 的完整设计基线：`docs/plan/makeup-agent-design.md`（2026-09-16）**。
    该文把「接生图引擎」与「多轮对话 agent」拆成两层（A 引擎 / B agent），给出 `LookSpec`
    结构化槽位、`prompt-builder` 确定性模板、`agent` 模块结构与 harness 八条、以及
    **`render_look` 必须人工确认触发**这条核心取舍。**开工前先读该文 §4**——
    它记着一条已花过钱的实测结论（身份保真取决于提示词里有没有构图/服装/背景条款），
    直接推出「**LLM 永远不写 prompt**」这条硬约束。
...
> ★★ **2026-09-16 落地了，但走的不是下面这条路。按现状读，别照下面的待办开工。**
>
> 那天拿到**真实品牌资料**（`products/YSL 全产品性质知识库(1).docx`，57 条 / 9 类目，
> 标题就写着「供AI引擎使用」），于是方案按**实物**重写，落地成**另一条路**：
>
> | | 原设想（下面那批待办） | 实际落地（2026-09-16） |
> | --- | --- | --- |
> | 匹配逻辑 | **规则引擎**：`occasion + 肤质/肤色 + 衣橱缺口` → 查表 | **模型自己读**：资料给模型，由它判断（人拍的板） |
> | 驱动输入 | 衣橱 `cabinet.listByUser`（「缺什么补什么」） | **用户主动问起产品**那一轮（★ 不是"妆容定下来就推"，见下） |
> | 数据 | 静态常量表，5 个场合量级 | **内容库**：`products/<库>/`，导入器生成、进 git |
> | 落点 | 结果页推荐区 | **对话页**（`AgentView.vue` 的「参考产品卡」） |
> | 模块 | 新建 `recommendations` 模块 | 新建 `products` 模块 + agent 两个只读工具 |
>
> 复用的只有一件：**端口声明在消费方 + `src/index.ts` 粘合**这条（§7.1），这次是第三处先例。
> 详见 `server/src/modules/products/README.md` 与 `server/scripts/README.md` 的 `import-products.ts` 一节。
>
> ★★ **触发条件是人拍板的：推荐是用户要的，不是妆容做完了我们该给的。**
> 但**「问」和「读」是两件事**，别写成一条：
> **问一句**（「要不要给你挑两支」）妆面定下来后**随时可以**（人原话：「可以主动挑话头」）；
> **读库**要**等用户开口**——自己问起，**或者答应了那一句**。
> 所以口径不是"妆面做完什么都不许提"，而是**"妆面做完只是问的时机，不是读的理由"**：
> 用户还没开口的那一轮，模型**不该先读出来备着**（真实退化路径就是"读了不一定推，
> 先备着总没错"，于是库内容提前进了上下文）。
> 这条同时钉在**提示词**与**两个工具的描述**里（只改一处，另一处会把模型拉回旧行为），
> `SYSTEM_PROMPT_VERSION` 由 `v3` 提到 `v4` 就是这次收窄。
> ⚠️ **两次错法的方向相反，两条都得防**：只写"等用户开口"→ 模型连问都不敢问
> （第一版就是这么写的，当天被人纠正）；只写"可以问一句"→ 它问完直接就去读了。
> 原口径（"用户想知道就推"）**看起来更聪明，实际会变成主动推销**，记录在此以免下次又被"优化"回去。
>
> ⚠️ **「别做成商品库」那句要按实物修正，但结论没作废、只是边界换了地方**：
> 现在的库是 **57 条真实产品**（远超原设想的常量表）——所以「规模」那半句已经不成立；
> 但它**依然不是商品库**，因为**不可交易**：没有价格、没有购买链接、没有库存、没有购物车，
> 它是一份**给模型读的品类资料**。要守住的是**这条边界**，不是那个条数。
>
> ⚠️ **没落地的部分别当成已完成**：**衣橱缺口那一套（`cabinet.listByUser` →「缺什么补什么」）
> 一行代码都没有。** 2026-09-16 这条路是**另一条**，不是它的替代——衣橱锚点那个已知代价
> （自由键值、没有稳定品类）原封不动还在（见下面「输入建模」那条）。
...
- **待办（2026-09-16 起分成两栏读，别把已落地的当待办重做）**：

  **【已落地】** —— 见上面那张对照表，实现在 `server/src/modules/products/` + agent 的两个工具。
  - [x] 新建模块，端口**重新声明在消费方**（§7.1），`src/index.ts` 粘合。★ 这次粘的是
    `products` 给 `agent`，**不是** `cabinet` 给 `recommendations`——粘法一样，两端都换了。
  - [x] **数据表**：不是静态常量表，是 `products/<库>/` 内容库（导入器生成、进 git、启动时急切加载）。
  - [x] **呈现守红线 §13-6**：角标「品牌参考」**由前端写死**（不取决于模型措辞）、
    「社交平台用户反馈摘要」必须说明出自品牌资料、空清单整块不渲染（宁可空着）。
  - [ ] **确认一遍**：不接第三方 ad SDK / 不做画像 / 不接购物记录（红线 §13-5）——**这几条至今没被碰过**，
    加产品库也没有破它们（内容库不可交易，见上）。有疑问就查 `products/README` 与 `.env.example`。

  **【仍未做】** —— 这几条**一行代码都没有**，别以为随 2026-09-16 那次一起做了。
  - [ ] **规则引擎**：`occasion + 肤质/肤色 + 已有品（cabinet.listByUser）` →「缺什么补什么」——
    **优先已有品，缺的推对应产品**。典型例：夏日海岛 + 高湿度 → 缺「高防水防晒 / 定妆喷雾」。
    ★ 2026-09-16 那条路是**模型自己读资料判断**，**不是**这条。两者不冲突，但**这条是空着的**。
  - [ ] `src/index.ts` 把 cabinet 的 `listByUser` 粘进来（粘法同 §1「跨模块问一句」）。
  - [ ] 结果页推荐区（现在推荐在**对话页**，结果页没有）。
  - [ ] 定入参出参形状：入参带「已拥有品」（形状取 cabinet 的 `CosmeticItemView[]`）；
    **出参要能装下「推的是什么 + 为什么推它 + 是品牌参考」**。
    （2026-09-16 那次**绕开了**这个设计：模型不返回结构化推荐，只把读过的产品记进
    `session.consultedProducts`，正文自由发挥。**代价**：我们只能保证"读过什么"可辨认，
    保证不了"他说推的是哪支"——角标盖住的是**卡片**，盖不住正文里那句话。要收紧就回到这条。）

    > ✏️ **2026-09-17：上面那句「盖不住正文里那句话」当场应验了，而且应验了两次。**
    > 一次真机实测跑通之后，用户在正文里看到两样东西：**产品名那个位置上写着 id**
    > （「`33-touche-clat-le-teint`（妍活青春/逆龄粉底液）」），以及**模型交代了自己的取数过程**
    > （「我马上从库里给你挑几支」）。两条都**只能靠措辞收**——正文本来就不受我们管，
    > 这正是本条记的那个缺口。
    > 处置：`SYSTEM_PROMPT_VERSION` 提到 `v13`（`system-prompt.ts` 的沿革），
    > 外加 `application/tools/list-products.ts` 的**版面**改动（id 从行首挪到行尾）。
    > ⚠️ **没有回到本条的「定入参出参形状」，所以这个缺口一个字都没变小**——
    > 只要正文还能自由发挥，**下一次它换个形状还会出现**。
    > ★ 判据是「正文里还会不会再冒出 id / 过程交代」，不是"提示词还能不能再收一收"。

  > ⚠️ **下面这条原有的「自有产品特性表」设想已被 2026-09-16 的实物取代**，保留是因为
  > 它记着两个**仍然成立**的边界（不接购物记录、不抓网络图）。**别再按"静态常量表"去实现它。**
  - ~~自有产品特性表~~（**已被内容库取代**）：原设想只收**赞助方（欧莱雅）旗下**产品、
    一条 = 品类/名称/能解决什么/适用肤质、**静态常量表**放 `domain/`、规模 5 个场合量级。
    ★ **边界照旧**：**不接淘宝等购物记录**（红线 §13-5 没变）；**自有产品表也不抓网络图**
    （§13-2 的「不抓网络图」已在 2026-09-11 为 `references` 模块推翻，但那是**参考图检索**这一个模块的
    知情变更，**没有**顺延到内容库的理由 —— 抓图只会引入新的授权面）。
    ★ **条数不再是约束**（实物 57 条），**"不可交易"才是**：无价格 / 无购买链接 / 无库存 / 无购物车。
...
- **接缝（将来）**：
  - [ ] **喂给推荐**：将来做平价推荐时，在 `src/index.ts` 把 `listByUser` 粘过去。
    ⚠️ **2026-09-16 那版推荐没走这条**（模型读内容库，不看衣橱），所以这个缝**仍然是空的**——见 §9。
...
- **待办 [ ]**：
  ...
  - [ ] **推荐 / 品牌参考位**（2026-09-10 拍板要做，见 §9）：结果页一块由「场合 + 衣橱缺口」推出来的推荐，
    **必须标「品牌参考」**；空着也比硬贴强（红线 §13-6）。
    ⚠️ **结果页这一块仍未做**：2026-09-16 落地的那版在**对话页**（`AgentView.vue` 的「参考产品卡」），
    而且驱动输入是"妆容定下来之后那句需求"，**不是衣橱缺口**。两件事别混（见 §9）。
...
2. **IP / 原创**：素材、参考图、模板字体逐张记录来源；~~不抓网络图~~。参考素材必须 自绘 / 自有 / 可授权。
   ...
   >
   > ⚠️ **上面那句引文只核到搜索引擎摘要这一层，不是主办方页面上的原字**（细节与核实过程见
   > `reference-fetch-feasibility.md` §6）。另一件要记住的：该条款的后果措辞是「**否则后果自负**」，
   > **不是**「直接出局」——本仓库一度写成后者（把「后果自负」加重成「直接出局」），已改回。
   > 待有账号的人登录天池读原文后更新。
   >
   ...
   >
   > **⚠️ 一条可能的出路（2026-09-11 记，尚未拍板）**：上妆引擎若走**参数化接口**，不吃参考图，
   > `references` 就不再是主链路的必需输入，可**降为可选**，本模块那个「没有合规素材来源」的悬空项随之消掉。
   > 但**这不是自动成立的**：只有当场拍板走那条腿、且确认产品叙事不需要参考妆回显时才算数。
   > **在那之前，本条仍按「未解决」对待，不得因为这条备注就停止追授权。** 见 `ai-engine-api-spike.md` §1.2。
   >
   > **⚠️ 2026-09-12：那条出路已随主选一起作废。** 上面说的「参数化接口」指的是 Perfect Corp
   > AI Makeup VTO，而该厂商**已因成本被放弃**。**出路不在厂商，在于「引擎收不收结构化参数」这个性质**
   > ——所以重开选型时，**「新候选是不是参数化接口、吃不吃参考图」应当作为一项硬性筛选条件提前问，**
   > 而不是等选完了再回头发现它又需要参考图。这是本节唯一一条能带到下一轮的结论。
   > **在那之前，本条按「完全未解决、且暂无出路」对待。** 见 `ai-engine-api-spike.md` §0.2。
3. **肤色 / 肤质包容**：`skinTone` 5 档、缺省 `medium`（中间档），**不默认浅肤色审美**；上妆与推荐都按真实肤色走。
   > **⚠️ 2026-09-12：这条从直觉升级为有引用的论据。**
   > **ICCCV 2026 偏差审计**（DOI 10.1145/3810417.3810425，论文 CC BY 4.0）拿 320 张按 4 族裔平衡的
   > 合成脸测 BeautyGAN / PSGAN / EleGANt，结论是 **Black-presenting 脸退化最严重**
   > ——错位、光度/色度伪影、**非预期肤色漂移**，并用 CIELAB **mid-face ΔL\*** 量化明度漂移。
   > **这是学术界唯一真正量化深肤色退化的工作**，可直接引作本红线的依据。
   >
   > **同时要记住反面，它决定了本条的验收方式**：2026 年所有方法论文都只说
   > "diverse skin tones"，**没有任何一篇给出「5 档肤色各自通过率」这种表**。
   > **也就是说这套评测口径学术界没有现成的，得自己建**——而 `ai-engine-api-spike.md`
   > §4.2 的 5 档夹具 + §4.4 评分表**正是这个口径**。主选作废后那套夹具仍要保留，理由在此。
   >
   > 另：某国货粉底推荐**深肤色适配率仅 27%**（较浅肤色低 53%）是行业侧的镜像数据，可作叙事对立面。
...
- [ ] 渲染方案 ① 自研参数化 vs ② 第三方 API（本周半天验证后拍板 → 决定 §6 的人脸关键点 / 渲染两单怎么派）。
  **2026-09-11 复核后收窄为**：③ 第三方**参数化**上妆接口 Perfect Corp AI Makeup VTO 成为 ① 的可行替代，
  同样是「结构化参数驱动」，但不用自建关键点检测 + 渲染。**这是目前最值得先花半天验证的分支**，见 §6 补充。
  **⚠️ 2026-09-12 推翻**：③ 的载体 Perfect Corp **已因成本被放弃**，选型**回到 ① / ② 的原始二分**
  ——即本项**回到 09-11 之前的状态**，但比那时候更糟：09-11 当时把 ② 的想法挂在了 Perfect Corp 上，
  现在 ② 也**没有具体候选**，要靠新一轮调研重新找。**本项现在是完全未决状态**，见 §6 补充与该文 §0。
  - **⚠️ 2026-09-12 二次更新（同日 owner 明确表态后）**：owner 原话「不要了，用不起」，
    确认了作废原因**就是费用**。于是 **② 这一支不只是「没有候选」，而是按成本整类关闭**——
    凡是按张 / 按次计费的商业 API 都在内。**① / ② 的二分因此塌缩为 ① 一支：自研参数化渲染。**
  - **① 现在有具体候选了**：`OpenMakeupSDK`（MIT、本地跑、零显存、零上传）**已经在仓库里，就是 `tespro/`**。
    它的审计未做（`ai-engine-makeup-models.md` §8 第 1 项），**未审计前不得当成已拍板**。
  - 注意区分：关闭的是**商业 API 这一类**，**不是**自研路线。`ai-engine-selfhost-review.md`
    对「整条扩散自建链」的排除**与本条无关**，那条排除仍然有效（理由里没有一条是价钱）。
  真正的开源自研 MAD / SD+FaceID / FLUX+LoRA 经复核**不进入候选**，见 `ai-engine-api-spike.md` §3.1；
  SDXL / ControlNet / IP-Adapter FaceID 整条自建链的完整论据见 **`ai-engine-selfhost-review.md`**，
  结论同样是不进主链路，但该文 **§5** 留了一份**在本机 8 GB 显存上真跑得起来**的实施方案，
  若有人想练手照那个来：**关键是砍掉 FaceID / InstantID 两条 ID 腿**，并把关键点检测器换成 mediapipe。
  > **这条排除结论**（自建链不进主链路）**本轮不受影响**——它是按「架构是反的 / 许可撞红线 /
  > 本机跑不动」三条判的，**一条都不是价钱**。但要注意：上述排除都建立在「云端有便宜好用的替代」这个
  > 隐含前提上。**替代品一旦不存在，自建链的相对分量就变了**——不是它的结论变错，而是
  > 「不选它」的机会成本变了。重开选型时，这一条**要重新过一遍**，不能直接沿用。
  > 同样要重新过一遍的还有 `ai-engine-selfhost-review.md` §6 的「云 API 是更省的替代」那张对比表——
  > 它整张表的右边一列（云 API 的 $0.03/chart 与免费额度）**正是被本轮否掉的那类东西**。
...
- [ ] （**2026-09-16 新开**，见 `makeup-agent-design.md` §14.1）**接引擎 + 做对话式 agent 的四个拍板项**：
  ① `LookSpec` 并入 `brief` 还是 `EngineInput` 新开字段；② 会话 TTL 取值与清理任务形式；
  ③ `render_look` 单会话调用上限；④ **`qwen-image-edit-plus` 与 DashScope 文本模型的计费是否接受**
  ——§14 那次「按成本整类关闭」针对的是 Perfect Corp 一类**美妆垂直 API**，
  **千问系列不在那次关闭范围内**，别默认它便宜，要 owner 确认一次。
  另有两格必须先实测再开工（该文 §14 阶段 0）：**真人自拍的平台人脸审核**、**DashScope 目标模型的 tool calling 是否真支持**。
