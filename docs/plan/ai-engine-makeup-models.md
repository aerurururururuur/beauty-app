...
1. ~~GitHub 开源项目全盘点没跑完，dlib 那个悬念没有结论。~~ **⚠️ 已打捞，结论是：**
   ...
   **→ 2026-09-13 已补完：见 `ai-engine-oss-landscape.md`**（A 类实时试妆全扫 + B 类生成式 +
   C 类商业 + 「换栈」方向）。该篇给出了本条的结论：**不存在更好的同类开源试妆 SDK，品类是空的**；
   并且**用 curl 绕开了 WebFetch 全域拦截**，把上一轮卡在 `[检索级]` 的许可/活跃度
   全部提成了 `[一手·API实测]`——§7 第 4 条列的那串"未核许可"里，凡在 GitHub 上的都可用此法补上。
