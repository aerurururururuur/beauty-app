...
<template>
  <div class="page home">
    ...
    <div class="cta-area">
      ...
      <button class="btn btn-ghost btn-block" @click="router.push('/agent')">
        <Icon name="sparkle" :size="16" />
        对话定妆
      </button>
      <!--
        ★ 这段文案不能只写"不收集任何照片"。
        加了「对话定妆」之后照片**真的会上传**到本项目自己的后端(`data/inputs/`),
        对话那条路还会把它留到空闲 TTL 到期(缺省 24 小时)。旧文案「全程浏览器本地演示,
        不收集任何照片」在**两条路上都不成立**(表单那条同样存在服务端)。
        这是 §10 [I8] 那条隐私红线的对外表述 —— 首页替系统说一句不真的话,
        比这个功能本身的问题更严重。规矩:**只说真的**。
      -->
      <p class="hint">
        衣橱里录上你已有的化妆品，配妆时会参考「已经有什么、还缺什么」。<br />
        照片会上传到本项目自己的后端，不是「只留在浏览器里」；<br />
        对话定妆那条路在闲置 24 小时后自动删除。<br />
        缺省引擎是骨架示例：它不改你的照片，也不会把照片发给第三方。
      </p>
    </div>
  </div>
</template>
