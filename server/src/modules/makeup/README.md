...
## ⚠️ `MAKEUP_ENGINE=qwen` 让表单路径**不可用**
...
- **现状**：`mock` 可用；`qwen` / `replay` 代码就位、单测绿，**但从未真跑过一次付费生成**（`__fixtures__/` 是空的）。
- **待办**：
  ...
  2. ✏️ **`data/engine-out/` 的清理：agent 那条路已经做了，`jobs` 那条路还没。**（2026-09-16）
     ...
     - ⚠️ **仍未解决的**：`jobs` 那条路（`run-pipeline` → `artifactStore.putResult`）**不走那个适配器**，它的中间产物照旧只增不减。`jobs` 已冻结（§8.1），而且 `qwen` 下表单路径**本来就不可用**（见本文件上面那节 / 设计文档 §8.1），所以这条**当前没有真实泄漏**——但**只要有人让表单路径重新能用，它就立刻变成一笔隐私债**。别当它已经关掉了。
