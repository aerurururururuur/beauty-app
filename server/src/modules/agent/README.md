...
✏️ **2026-09-16 起有两条入口**（下面那张图是入口 A；入口 B 见紧随其后的一段）：
...
模型调 render_look
  ...
  → 前端把 pendingRender.summary 摆成对话里那条出图消息（模型不转述，见下一节）
用户点那个按钮 → POST /agent/sessions/:id/render
  → 服务端把**那一整轮**重放一遍，这次 `confirmation='approved'` → 真出图
用户继续说别的 / 什么都不点 → 就是不发这条请求（缺省即 `declined`）

【入口 B —— ✏️ 用户点界面上那条「确认生成」】
妆面定了 + 照片有了 + 没有提议欠着（视图给出 renderOffer）
  → 那条消息是**界面按状态自己摆的**，与模型说不说话无关
用户点它 → POST /agent/sessions/:id/render（**同一条路由、同一个请求体**）
  → 服务端**代递**一条 assistant 轮 {tool_use, id:'manual-<uuid>', name:'render_look'}
  → 再按 'approved' 跑 —— 同一次请求里就出图
```

**为什么入口 B 必须有**：在此之前出图**只有入口 A 一条链**，而"模型提不提议"是**提示词级**的事。
2026-09-16 实测里真实模型在用户**两轮明确要图**时一次都没调 `render_look`
（「生成图片我看看」→ 反问要不要生成；「出图」→ 又把妆面讲了一遍），
于是界面上**既没有图、也没有那个确认框**——唯一的入口断了，整条路就没了。
那条链现在改由**状态**驱动：`renderOffer` 只在妆面照片齐、没有待确认时出现，
额度用尽时连按钮都不给。★ **提示词一个字没改**（规则 6 与 `SYSTEM_PROMPT_VERSION` 仍是 `v11`）。
> ✏️ **2026-09-17:** 上面这一句说的是**2026-09-16 那次改动**（"出图不必由模型转达"），
> **别再读成"现在仍是 `v11`"**。同日之后提示词改到了 **`v12`**，动的是**规则 3**
> ——见 `application/system-prompt.ts` 的沿革与 `scripts/probe-agent-prompt.ts`。
> **本节那条链一个字没动。**
...
- **闸门不在模型手里。** `confirmation` 只由确认那条 HTTP 路径（和它的 `declined` 兜底）设置。
  ...
  ✏️ 顺带说明**这条路由为什么值钱**：它现在也是"用户要图"这件事**唯一不必经过模型**的一条路，
  而那个"确认"的扳机就是用户点自己屏幕上那条消息。模型不开口，路也通。
- **`pendingRender` 存进会话视图，不只活在 `events[]` 里。** 它是一个还没答的 `tool_use`
  这个**派生状态**（不存第二遍），刷新页面后确认框因此还在。
  ✏️ 视图里现在是**两个互斥的**出图入口：`pendingRender`（模型提的）或 `renderOffer`
  （界面自己摆的）。`renderOffer` 只在**没有待确认调用**时才出现——两条同时给出，
  页面上就是两个按钮指向同一次花钱，其中一个必然 422。
- **重放那一整轮，而不是只跑工具。** `[C]` 要求同轮结果装进同一条 user 消息，
  ...
  ✏️ **入口 B 走的是同一条重放链**：它把"代递的那条提议"当成欠账，于是额度、引擎、
  先落盘再记会话、`addRender`、`seq`、事件协议**一行都没有抄第二份**。
  那条代递的消息（`manual-<uuid>`）**只活在这一次 `execute` 里，绝不单独落库**——
  加了中间 save，用户刷新就会看见一个"模型从没提过的确认框"。
- **额度查两遍。** 提议时查一遍（前端因此拿不到一个点下去必然失败的确认框），
  `'approved'` 分支**再查一遍**——提议与确认之间隔着一次用户往返，期间他完全可能又出了一张。
  ✏️ 入口 B **没有提议阶段**，所以它只吃到第二遍。这是安全的，因为视图的 `renderOffer`
  本来就在额度用尽时不给按钮（`left === 0` 就只剩一行说明）。★ 判据只留一份：
  "缺什么才算不能出图"是 `session.ts` 的 `renderReadiness()`，工具与用例各写面向自己读者的文案。
- **✏️ 同一个会话同时只能有一个人在出图。** `ConfirmRender` 里一个按会话的进程内
  `Set<string>` 锁（`finally` 清）。没有它，连点两次就是**两个请求都调引擎、都算出同一个
  `seq`**：出两张图扣两次钱，只留一条记录。⚠️ 这是**缓解，不是"已经安全了"**——
  残余空洞：**响应在回程丢了**、用户看着屏幕上那条消息又点一次，在服务端看来是一个
  **正当的新请求**，会再出一张。要关掉它得有幂等键，而幂等键要在这条
  「一个出图参数都不收」的路由上**加客户端输入**——**不为这个洞破例**。
  （同样的取舍与理由见 `application/usecases/confirm-render.ts` 的「残余空洞」。）
...
- **那句话不再由模型说。** 那条出图消息是界面自己摆的（✏️ 2026-09-16 起就在对话流里，
  不是弹在旁边的框），文案是 `renderConfirmationSummary` 自己写的（唯一来源），
  模型既不转述也不交代它的状态。**没有义务，就没有"说错时机"。**
...
- ✏️ **2026-09-16：这一支又复现了，而且这次照 `v7` 立的规矩办了——改的是代码。**
  复现的形状是第四种：**用户两轮明确要图，模型一次都没调 `render_look`**，
  正文也没有转述什么，就是**不提议**。四轮改措辞改的全是"已经提议之后怎么说"，
  而"提议"这个动作本身**从头到尾都只是提示词里的请求**。所以扳机从提示词挪到了
  界面（入口 B，见本节开头）——**不再请求模型别说，而是让"用户要图"不必由它转达**。
  ★ 提示词正文与 `SYSTEM_PROMPT_VERSION` **一个字没动**（仍 `v11`）。
  > ✏️ **2026-09-17:** 同上——这句记的是**那一次**改动；提示词当天晚些时候改到了 `v12`（规则 3）。
...
✏️ 2026-09-16：**"必须经用户确认"没变，变的是"谁来提这件事"。** 上面那三个前置条件
（外加"上一轮欠着的必须正是出图请求"）现在是 `session.ts` 的 `renderReadiness()` 一个判据，
`render-look.ts`（写给模型看的那两句）与 `ConfirmRender`（写给用户看的那两句）各取所需；
模型**不提议也拦不住用户出图**（入口 B）。★ 但这也意味着"模型提议 → 用户批准"这一对
**仍然是唯一的路径**：入口 B 是**服务端代模型递一条提议**再按 approved 跑，
所以模型的历史里永远有一对 请求→批准，与它被交付的规则 6 对得上。
...
- [x] ✅ **真模型一个工具都不调 —— 2026-09-17 修掉，提示词 `v12`，只动规则 3。**
      ...
      **把整轮用来反问**，工具一次都没轮到。
      ★ 修法（实测选的，不是想当然）：只把后半句从"否则就不动"改成"**问的同时把这一轮该做的做完**"，
      并说清**妆面那一步本来就不需要肤色**（`propose_look` 里没有这个字段）。
      ⚠️ **试过但没采用的两种**：加"点名「偏深」不算说清楚"→ 模型退回只反问；
      只写"照常往下走"→ 3/3 但要靠**替用户猜肤色**换来的（`patch_brief` 写死 `skinTone`，
      `propose_look` 被按肤色收窄的校验器打回再重试）。**那半个命中率是假的**，理由与数据
      见 `application/system-prompt.ts` 的 `v12` 沿革。
      ★ 复现与回归用 `npm run probe:agent-prompt`（脚本 `scripts/probe-agent-prompt.ts`，**跑真循环**）；
      `--verify v12d` 一次调用都不发，核对"代码里现在这一段 == 当时测的那一段"。
      ⚠️ **仍有 1/3 落空**，形状是"调了 `patch_brief`、正文里把妆面讲成已定"——
      那一支够不着本版（不在"问吃掉整轮"这条线上），但**可自愈**（下一轮 `describeLookState` 会纠正）。
