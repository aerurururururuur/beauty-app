...
这是 roadmap §9「用户『已拥有产品』从哪来」的答案——推荐模块将来按 `userId` 经 `listByUser`
拿这份清单，做「缺什么补什么：优先已有品，缺的推平价线」。
...
- **推荐模块**：`recommendations` 的 `RecommendInput` 需要加「已拥有品」，`RecommendationItem`
  现在只有 `{ id, name, note }`、撑不起「品牌参考」。**本轮未动 recommendations 代码**，
  落地时在 `src/index.ts` 把 `cabinet.listCosmetics`（或直接 `items.listByUser`）注入即可。
